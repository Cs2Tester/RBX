...
function library:Window(name)
    ...
    WindowStroke.Parent = UiWindow
    WindowStroke.Color = Color3.new(1, 1, 1)
    ...
    Minimise.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
    ...
    local MinimiseStroke = Instance.new("UIStroke")
    MinimiseStroke.Parent = Minimise
    MinimiseStroke.Color = Color3.new(1, 1, 1)
    MinimiseStroke.Thickness = 1
    ...
    local WindowInnerStroke = Instance.new("UIStroke")
    WindowInnerStroke.Parent = Window
    WindowInnerStroke.Color = Color3.new(1, 1, 1)
    WindowInnerStroke.Thickness = 1
    ...
    function functions:Button(text, callback)
        ...
        Button.MouseEnter:Connect(function()
            ...
            hoverTween = TweenService:Create(Button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 40)})
            hoverTween:Play()
        end)
        Button.MouseLeave:Connect(function()
            if hoverTween then hoverTween:Cancel() end
            hoverTween = TweenService:Create(Button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(20, 20, 20)})
            hoverTween:Play()
        end)
    ...
    function functions:Toggle(text, on, callback)
        ...
        ToggleDescription.TextColor3 = Color3.new(1, 1, 1)
    ...
    function functions:Box(text, callback)
        ...
        BoxDescription.TextColor3 = Color3.new(1, 1, 1)
    ...
    function functions:Slider(text, min, max, default, callback)
        ...
        min = min or 1
        max = max or 100
        default = default or (max - min) / 2 + min
        callback = callback or function() end
        local offset = 70
        ...
        local function updateSliderFromMouse(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                local dragging = true
                local delta1 = SliderButton.AbsolutePosition.X - SliderButton.Position.X.Offset
                local conn
                conn = stepped:Connect(function()
                    if dragging then
                        local xOffset = mouse.X - delta1 - 3
                        xOffset = math.clamp(xOffset, 0, 175)
        ...
        Slider.InputBegan:Connect(updateSliderFromMouse)
        ...
        SliderButton.InputBegan:Connect(updateSliderFromMouse)

        local SliderButtonStroke = Instance.new("UIStroke")
        SliderButtonStroke.Parent = SliderButton
        SliderButtonStroke.Color = Color3.new(1, 1, 1)
        ...
        SilderFiller.BackgroundColor3 = Color3.new(1, 1, 1)
    ...
    function functions:Dropdown(text, buttons, callback, selective)
        ...
        DownSign.Text = "^"
        DownSign.TextColor3 = Color3.new(1, 1, 1)
        ...
        function dropFunctions:Button(name)
            ...
            Button_2.MouseEnter:Connect(function()
                if hoverTween then hoverTween:Cancel() end
                hoverTween = TweenService:Create(Button_2, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 40)})
                hoverTween:Play()
            end)
            Button_2.MouseLeave:Connect(function()
                if hoverTween then hoverTween:Cancel() end
                hoverTween = TweenService:Create(Button_2, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(20, 20, 20)})
                hoverTween:Play()
            end)

            canvasSize = canvasSize + 27
            DropdownFrame.CanvasSize = UDim2.new(0, 182, 0, canvasSize + 1)
            if #DropdownFrame:GetChildren() - 1 < 8 then
    ...
    function functions:ColorPicker(name, default, callback)
        ...
        ColorPicker.Text = ""
        ColorPicker.TextColor3 = Color3.new(0, 0, 0)
        ColorPicker.TextSize = 14
        ColorPicker.ZIndex = 2 + zindex
        ColorPicker.BackgroundColor3 = default or Color3.new(1, 1, 1)

        local ColorPickerStroke = Instance.new("UIStroke")
        ColorPickerStroke.Parent = ColorPicker
        ColorPickerStroke.Color = Color3.new(1, 1, 1)
        ...
        PickerDescription.TextColor3 = Color3.new(1, 1, 1)
        ...
        local ToggleRGBStroke = Instance.new("UIStroke")
        ToggleRGBStroke.Parent = ToggleRGB
        ToggleRGBStroke.Color = Color3.new(1, 1, 1)
        ToggleRGBStroke.Thickness = 1

        ToggleFiller_2.Name = "ToggleFiller"
        ToggleFiller_2.Parent = ToggleRGB
        ToggleFiller_2.BackgroundColor3 = Color3.new(1, 1, 1)
        ...
        CloseStroke.Parent = ClosePicker
        ...
        ToggleRGB.MouseButton1Down:Connect(function()
            ...
            if ToggleFiller_2.Visible then
                rainbowConnection = stepped:Connect(function()
                    if ToggleFiller_2.Visible then
                        local hue2 = tick() % 5 / 5
        ...
        Canvas.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                local dragging = true
                local delta = Cursor.AbsolutePosition - Vector2.new(Cursor.Position.X.Offset, Cursor.Position.Y.Offset)
                local conn
                conn = stepped:Connect(function()
                    if dragging then
                        local mousePos = Vector2.new(mouse.X, mouse.Y) - delta
                        local x = math.clamp(mousePos.X, 2, Canvas.AbsoluteSize.X - 2)
                        local y = math.clamp(mousePos.Y, 2, Canvas.AbsoluteSize.Y - 2)
        ...
        Color.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                local dragging = true
                local delta1 = ColorSlider.AbsolutePosition.X - ColorSlider.Position.X.Offset
                local conn
                conn = stepped:Connect(function()
                    if dragging then
                        local colorPos = Color.AbsolutePosition
                        local colorSize = Color.AbsoluteSize
                        hue = 1 - math.clamp(1 - ((mouse.X - colorPos.X) / colorSize.X), 0, 1)
                        CanvasGradient.Color = ColorSequence.new{
                            ColorSequenceKeypoint.new(0.00, Color3.fromHSV(hue, 1, 1)),
                            ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))
                        }

                        local xOffset = math.clamp(mouse.X - delta1, 0, Color.AbsoluteSize.X - 3)
        ...
        function colorFuncs:UpdateColorPicker(color)
            ...
            elseif color and type(color) == "boolean" and not rainbowConnection then
                ToggleFiller_2.Visible = true
                rainbowConnection = stepped:Connect(function()
                    if ToggleFiller_2.Visible then
                        local hue2 = tick() % 5 / 5
