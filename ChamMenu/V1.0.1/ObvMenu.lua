...
local ESPModule = loadstring(game:HttpGet("https://cdn.discordapp.com/attachments/1451446961747132568/1451470513233854586/ObvCham.lua?ex=69464ac1&is=6944f941&hm=713b0ecf48fba03fdec6071e770a3441d99b9db42a3b0552aca54a2833e0e9f4&"))()
local AimFunctions = loadstring(game:HttpGet("https://cdn.discordapp.com/attachments/1451446961747132568/1451470534855491657/ObvAim.lua?ex=69464ac6&is=6944f946&hm=a210f9852c56c6b1d7e685d3dabce6f44102e579edd98a498eee4092bf98387b&"))()

local ESP = ESPModule.getESP()
local Materials = ESPModule.Materials

local FOV = AimFunctions.GetFOVSettings() or {Enabled = true, Radius = 100, Color = Color3.fromRGB(255, 255, 255), Visible = true}
...
createToggle(aimbotScroll, "Team Check", Aimbot.TeamCheck, function(value)
    Aimbot.TeamCheck = value
    if AimFunctions then
        AimFunctions.ToggleTeamCheck()
    end
end)

createToggle(aimbotScroll, "Visible Check", Aimbot.VisibleCheck, function(value)
    Aimbot.VisibleCheck = value
    if AimFunctions then
        AimFunctions.ToggleVisibleCheck()
    end
end)
...
createToggle(fovScroll, "FOV Circle", FOV.Enabled, function(value)
    FOV.Enabled = value
    FOV.Visible = value
    AimFunctions.UpdateFOVSettings(FOV)
end)
...
task.spawn(function()
    wait(1)
    AimFunctions.updateFOVCircle()
end)

task.spawn(function()
    wait(0.5)
    local refreshedFOV = AimFunctions.GetFOVSettings()
    if refreshedFOV then
        FOV = refreshedFOV
    end
end)
